...
def non_test_function(X):
    if X.shape[1] != 5:
        raise ValueError("输入 X 必须有 5 列。")
    x1 = X[:, 0]
    x2 = X[:, 1]
    x3 = X[:, 2]
    x4 = X[:, 3]
    x5 = X[:, 4]
    term1 = np.sin(np.pi * x1)
    term2 = np.cos(2 * np.pi * x2)
    term3 = x3**2
    term4 = np.exp(-x4)
    term5 = x5
    condition = (x1 + x2) > 1
    term_conditional = np.where(condition, x3, 0)
    f = term1 + term2 + term3 + term4 + term5 + term_conditional
    y = 2 * (f - 1)
    return y
...
def evaluate_model(X_train, u_train, Testx, Testy, target_value):
    ...
    mask = (Testy >= target_value - 0.5) & (Testy <= target_value + 0.5)
    if mask.any():
...
def save_progress(n_initial, repeat, method, methods_to_run):
    ...
    with open('progressall5d+10.json', 'w') as f:
        json.dump(progress, f)

def load_progress():
    if os.path.exists('progressall5d+10.json'):
        with open('progressall5d+10.json', 'r') as f:
...
def main(methods_to_run=['PM', 'LHS', 'G_opt', 'D_opt', 'K_means']):
    ...
    n_iterations = 10
    n_preselect = 10
    n_initial_values = range(90, 91, 10)
    n_repeats = 30
    excel_filename = '5d+10五种方法结果.xlsx'
...
if __name__ == "__main__":
    # 示例：只运行 PM 和 LHS 方法
    main(methods_to_run=['PM', 'LHS', 'G_opt', 'D_opt', 'K_means'])
