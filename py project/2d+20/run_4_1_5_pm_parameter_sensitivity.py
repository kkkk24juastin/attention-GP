...
RESULT_FILE = RESULT_DIR / "4_1_5_pm_parameter_sensitivity.xlsx"
...
EXPERIMENTS = (
    {
        "experiment": "n_preselect_sensitivity",
        "parameter_name": "n_preselect",
        "parameter_value": 10,
        "variant": "PM_full",
        "n_added": BASE_N_ADDED,
        "n_preselect": 10,
    },
    {
        "experiment": "n_preselect_sensitivity",
...
def select_next_index(x_train, y_train, x_pool, spec):
    ...
    n_preselect = min(int(spec["n_preselect"]), len(x_pool))
...
def run_pm(initial_x, initial_y, pool_x, pool_y, spec):
    x_train = initial_x.copy()
    y_train = initial_y.copy()
    x_pool = pool_x.copy()
    y_pool = pool_y.copy()

    for _ in range(int(spec["n_added"])):
        selected_idx = select_next_index(x_train, y_train, x_pool, spec)
...
def task_id(spec, n_initial, repeat):
    return (
        f"{SECTION}|{spec['experiment']}|{spec['parameter_name']}="
        f"{spec['parameter_value']}|n{int(n_initial)}|r{int(repeat)}"
    )
...
def build_tasks(done, shared_data):
    ...
    for spec in EXPERIMENTS:
        for n_initial in N_INITIAL_VALUES:
            for repeat in range(1, REPEATS + 1):
                tid = task_id(spec, n_initial, repeat)
                if tid in done:
                    continue
                split = get_split(shared_data, n_initial, repeat)
                tasks.append((tid, spec, n_initial, repeat, split))
    return tasks


def run_task(task):
    tid, spec, n_initial, repeat, split = task
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"tgp_{CASE_NAME}_{SECTION}_{spec['experiment']}_{n_initial}_{repeat}_"
    ) as tmp_dir:
        os.chdir(tmp_dir)
        robjects.r.assign("tgp_worker_dir", tmp_dir)
        robjects.r("setwd(tgp_worker_dir)")
        try:
            final_x, final_y = run_pm(initial_x, initial_y, pool_x, pool_y, spec)
    ...
    return {
        ...
        "parameter_value": float(spec["parameter_value"]),
        "target_value": TARGET_VALUE,
        "target_band": TARGET_BAND,
        "n_initial": int(n_initial),
        "n_added": int(spec["n_added"]),
        "n_preselect": int(spec["n_preselect"]),
        "tau1": TAU1,
        "tau2": TAU2,
        "repeat": int(repeat),
        "seed": int(seed),
...
def metadata_frame():
    return pd.DataFrame(
        [
            ...
            {"key": "base_n_added", "value": BASE_N_ADDED},
            {"key": "base_n_preselect", "value": BASE_N_PRESELECT},
            {"key": "tested_parameter", "value": "n_preselect"},
            {
                "key": "tested_n_preselect_values",
                "value": ",".join(str(int(e["n_preselect"])) for e in EXPERIMENTS),
            },
            {"key": "n_initial_values", "value": ",".join(map(str, N_INITIAL_VALUES))},
            {"key": "repeats", "value": REPEATS},
            ...
            {"key": "tau1", "value": TAU1},
            {"key": "tau2", "value": TAU2},
            {"key": "experiments", "value": ",".join(e["experiment"] for e in EXPERIMENTS)},
...
def write_workbook(raw):
    ...
    raw = raw.sort_values(["experiment", "parameter_value", "n_initial", "repeat"])
    counts = (
        raw.groupby(
            ["section", "case", "method", "experiment", "parameter_name", "parameter_value", "n_initial"],
            dropna=False,
        )
        .size()
        .reset_index(name="n")
        .sort_values(["experiment", "parameter_value", "n_initial"])
    )
    by_n = metric_summary(
        raw,
        [
...
def main():
    ...
    if not tasks:
        print(f"{SECTION} {CASE_NAME}: no pending PM parameter sensitivity tasks.")
        if RESULT_FILE.exists():
            write_workbook(pd.read_excel(RESULT_FILE, sheet_name="raw"))
        return

    print(f"{SECTION} {CASE_NAME}: run {len(tasks)} pending PM parameter sensitivity tasks.")
