...
RESULT_FILE = RESULT_DIR / "4_1_4_pm_target_sensitivity.xlsx"

SAMPLING_TARGET_VALUES = (0.0, 1.2, 1.3, 1.5, 1.6, 1.7)
EVALUATION_TARGET_VALUE = 1.5
TARGET_BAND = 0.2
...
def space_attention_weights(candidates, x_train, tau=TAU2):
    space_scores = np.min(cdist(candidates, x_train), axis=1)
...
def run_pm(initial_x, initial_y, pool_x, pool_y, target_value):
    ...
    for _ in range(N_ADDED):
        ...
        space_weights = space_attention_weights(x_pool[top_indices], x_train, tau=TAU2)
        selected_idx = int(top_indices[np.argmax(space_weights)])
...
def evaluate_final(x_train, y_train, test_x, test_y, evaluation_target_value):
    ...
    mask = (test_y >= evaluation_target_value - TARGET_BAND) & (
        test_y <= evaluation_target_value + TARGET_BAND
    )
    rmse_target = (
        np.sqrt(np.mean((pred_y[mask] - test_y[mask]) ** 2)) if mask.any() else np.nan
    )
    final_ql = quality_loss_scores(pred_y, pred_var, evaluation_target_value)
    best_idx = int(np.nanargmin(final_ql))
    return {
        "RMSE_all": rmse_all,
        "RMSE_target": rmse_target,
        "best_pred_quality_loss": float(final_ql[best_idx]),
        "best_true_target_error": float((test_y[best_idx] - evaluation_target_value) ** 2),
    }


def task_id(sampling_target_value, n_initial, repeat):
    return (
        f"{SECTION}|PM|sampling_T={sampling_target_value:.1f}|"
        f"eval_T={EVALUATION_TARGET_VALUE:.1f}|n{int(n_initial)}|r{int(repeat)}"
    )
...
def build_tasks(done, shared_data):
    tasks = []
    for sampling_target_value in SAMPLING_TARGET_VALUES:
        for n_initial in N_INITIAL_VALUES:
            for repeat in range(1, REPEATS + 1):
                tid = task_id(sampling_target_value, n_initial, repeat)
                if tid in done:
                    continue
                split = get_split(shared_data, n_initial, repeat)
                tasks.append((tid, sampling_target_value, n_initial, repeat, split))
    return tasks


def run_task(task):
    tid, sampling_target_value, n_initial, repeat, split = task
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"tgp_{CASE_NAME}_{SECTION}_T{sampling_target_value}_{n_initial}_{repeat}_"
    ) as tmp_dir:
        os.chdir(tmp_dir)
        robjects.r.assign("tgp_worker_dir", tmp_dir)
        robjects.r("setwd(tgp_worker_dir)")
        try:
            final_x, final_y = run_pm(
                initial_x, initial_y, pool_x, pool_y, sampling_target_value
            )
            metrics = evaluate_final(
                final_x, final_y, test_x, test_y, EVALUATION_TARGET_VALUE
            )
        finally:
    ...
    return {
        ...
        "case": CASE_NAME,
        "method": "PM",
        "parameter_name": "sampling_target_value",
        "parameter_value": float(sampling_target_value),
        "sampling_target_value": float(sampling_target_value),
        "evaluation_target_value": EVALUATION_TARGET_VALUE,
...
def metadata_frame():
    return pd.DataFrame(
        [
            ...
            {
                "key": "sampling_target_values",
                "value": ",".join(map(str, SAMPLING_TARGET_VALUES)),
            },
            {"key": "evaluation_target_value", "value": EVALUATION_TARGET_VALUE},
            {"key": "target_band", "value": TARGET_BAND},
            {
                "key": "target_rmse_region",
                "value": (
                    f"[{EVALUATION_TARGET_VALUE - TARGET_BAND}, "
                    f"{EVALUATION_TARGET_VALUE + TARGET_BAND}]"
                ),
            },
...
def write_workbook(raw):
    ...
    raw = raw.sort_values(["sampling_target_value", "n_initial", "repeat"])
    counts = (
        raw.groupby(
            [
                "section",
                "case",
                "method",
                "sampling_target_value",
                "evaluation_target_value",
                "n_initial",
            ],
            dropna=False,
        )
        .size()
        .reset_index(name="n")
        .sort_values(["sampling_target_value", "n_initial"])
    )
    by_n = metric_summary(
        raw,
        [
...
def main():
    ...
    if not tasks:
        print(f"{SECTION} {CASE_NAME}: no pending PM target sensitivity tasks.")
        if RESULT_FILE.exists():
            write_workbook(pd.read_excel(RESULT_FILE, sheet_name="raw"))
        return

    print(f"{SECTION} {CASE_NAME}: run {len(tasks)} pending PM target sensitivity tasks.")
