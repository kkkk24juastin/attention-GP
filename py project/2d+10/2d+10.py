...
def save_progress(n_initial, repeat, method, methods_to_run):
    ...
    with open('progressall2d+10.json', 'w') as f:
        json.dump(progress, f)

def load_progress():
    if os.path.exists('progressall2d+10.json'):
        with open('progressall2d+10.json', 'r') as f:
...
def main(methods_to_run=['PM', 'LHS', 'G_opt', 'D_opt', 'K_means']):
    ...
    n_iterations = 10
    n_preselect = 25
    n_initial_values = range(30, 91, 10)
    n_repeats = 30
    excel_filename = '2d+10五种方法结果.xlsx'
