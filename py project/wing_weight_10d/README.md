...
- `run_ga_optimization.py`: reads saved training sets, fits TGP, and runs GA optimization.
...
2. `run_ga_optimization.py` reads those saved training sets, fits a fresh TGP
   surrogate for each method/repeat, runs continuous real-valued GA, evaluates
   the GA optimum with the true analytical function, and writes
...
- GA optimized `ga_pred_quality_loss` as the main optimization QL;
...
saved training set is used to fit a TGP surrogate, and GA searches the design
space for the point minimizing predicted quality loss, `(mu - T)^2 + variance`.
The resulting design is then evaluated with the true analytical Wing Weight
function to report its real response and true quality loss. With `REPEATS = 20`,
each method produces 20 saved training sets and therefore 20 GA optima.
